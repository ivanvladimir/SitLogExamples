...
nth(1,[A|R],A,R).
nth(2,[A|[B|R]],B,[A|R]).
nth(3,[A|[B|[C|R]]],C,[A|[B|R]]).
nth(4,[A|[B|[C|[D|R]]]],D,[A|[B|[C|R]]]).


%indicamos pocisiones de los objetos a identificar.
pos(one,pos1,[1,1.2,0]).
pos(two,pos2,[1,1,0]).
pos(three,pos3,[1,0.5,0]).
pos(four,pos4,[1,0.3,0]).
...
averigua_pos(Id):-
  pos(Id,_,Pos),
  assign_func_value(go(Pos,Id))
.

averigua_pos2(Id1,Id):-
	pos(Id,_,Pos),
	assign_func_value(go2(Id1,Pos,Id)).

%inidcamos las posibles combinaciones.
...
compara_card2(C1,C2):-
   ( C1 == C2 ->
	  ...
	  | otherwise ->
	  Res= impar),
         assign_func_value(adversary(Res)).



random1(Y,Z):-
	get_random_element(Y,Z),
	assign_func_value(Z).
random2(Y,Z):-
	get_random_element2(Y,Z),
	assign_func_value(Z).


guess_card(C1,C2,C3,C4):-
  ( C1 == C2 ->
     object(C1,NC),
     Res = guess(NC,one,two)
  | C1 == C3 ->
     object(C1,NC),
     Res = guess(NC,two,four)
   | C1 == C4 ->
     object(C1,NC),
     Res = guess(NC,two,three)
   | C2 == C3->
      object(C2,NC),
     Res = guess(NC,one,four)
   | C2 == C4->
      object(C2,NC),
      Res = guess(NC,one,three)
   | C3 == C4->
       object(C3,NC),
       Res =guess(NC,one,two)
   | otherwise ->
...
guess_card2(C1,C2,C3,C4):-
  ( C1 == C2 ->
     ...
     Res = sure(C1,one,two,Next,three, four)
  | C1 == C3 ->
      (C2 == two ->
       Next=C4
      |otherwise->
     ...
     Res = sure(C1,one,three,Next,two,four)
   | C1 == C4 ->
      (C3 == three ->
       Next=C2
      |otherwise->
     ...
     Res = sure(C1,one,four,Next,two,three)
   | C2 == C3->
      (C1 == one ->
       Next=C4
      |otherwise->
     ...
     Res = sure(C2,two,three,Next,one,four)
   | C2 == C4->
       (C1 == one ->
       Next=C3
      |otherwise->
      ...
      Res = sure(C2,two,four,Next,one,three)
   | C3 == C4->
     (C1 == one ->
       Next=C2
      |otherwise->
       ...
       Res =sure(C3,three,four,Next,one,two)
   | otherwise ->
       Res = sure(nada)
   ),
   assign_func_value(Res).
