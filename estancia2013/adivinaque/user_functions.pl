...
object(coke,can).
object(coke,bottle).
object(sprite,drinks).
object(sprite,can).
object(sprite,green).
object(sprite,bottle).
object(fanta,orange).
object(fanta,drinks).
object(fanta,can).
object(fanta,bottle).
...
object(water,drinks).
object(water,bottle).
object(water,transparent).
object(pepsi,black).
object(pepsi,drinks).
object(pepsi,bottle).
object(pepsi,can).
object(juice,cartoon).
object(juice,drinks).
object(juice,red).
object(juice,orange).
object(seven-up,bottle).
object(seven,drinks).
object(seven-up,can).
object(seven-up,green).
object(milk,white).
object(milk,cartoon).
object(milk,drinks).
object(drops,box).
...
object(drops,small).
% drink preguntas
pregunta(red,'is it red').
pregunta(green,'is it green').
pregunta(black,'is it black').
pregunta(orange,'is it orange').
pregunta(white,'is it white').
pregunta(drinks,'is it a drink').
pregunta(can,'is it a can').
pregunta(cartoon,'is it a cartoon').
pregunta(bottle,'is it a bottle').
pregunta(transparent,'is it transparent').
pregunta(blue, 'is it blue').
pregunta(small,'is it small').
pregunta(bos,'is it a box').
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Adivina Que  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
% Funciones de Adivina Quien 
put_elements([],LL,LL).
put_elements([Obj|Objs],Acc,LL):-
   put_elements(Objs,[Obj|Acc],LL).

exclude_feats(_,[],NL,NL).
exclude_feats(Feats,[object(_,Feat)|Rest],Acc,NL):-
    member(Feat,Feats),
    exclude_feats(Feats,Rest,Acc,NL).
exclude_feats(Feats,[Obj|Rest],Acc,NL):-
    exclude_feats(Feats,Rest,[Obj|Acc],NL).
    

get_object(_,[],Acc,Acc).
get_object(FF,[Obj|Objs],Acc,Lista):-
  findall(X,object(Obj,X),Ltmp),
  print(nl),
  print('HOLA2'),
  print(Ltmp),
  exclude_feats(FF,Ltmp,[],Ltmp2),
  print('HOLA3'),
  put_elements(Ltmp2,Acc,Acc2),
  get_object(FF,Objs,Acc2,Lista). 


set_random(X):-
  pid(PID),
  (PID > 30000 ->
  	setrand(rand(30000,30000,122))
  | otherwise ->
  	setrand(rand(PID,PID,122))),
  assign_func_value(empty).
 

%Las cajas de magia de adivina Que.
make_questions(FF,Objs):-
  print('---------------------------------------'),
  print(Objs),
  get_object(FF,Objs,[],L),
  length(L,Size),
  random(1, Size, RVal),
  nth(RVal,L,Feat),
  pregunta(Feat,Pregunta),
  print(Pregunta),
  print(nl),
  assign_func_value(sabe(Pregunta,Objs,Feat)).


create_list([],_,NL,NL).
create_list([Obj|Rest],F,Acc,NL):-
   object(Obj,F),
   create_list(Rest,F,[Obj|Acc],NL).
create_list([Obj|Rest],F,Acc,NL):-
   create_list(Rest,F,Acc,NL).

%Funcion para evaluar Respuesta
descartar_pregunta(Objs,Feat,si):-
  create_list(Objs,Feat,[],NObjs),
  print(nl),
  print('AQUI ESTOY'),
  print(NObjs),
  assign_func_value(continua(NObjs)).


exclude_list([],_,NL,NL).
exclude_list([Obj|Rest],F,Acc,NL):-
   object(Obj,F),
   exclude_list(Rest,F,Acc,NL).
exclude_list([Obj|Rest],F,Acc,NL):-
   exclude_list(Rest,F,[Obj|Acc],NL).



%Funcion para evaluar Respuesta
descartar_pregunta(Objs,Feat,no):-
  exclude_list(Objs,Feat,[],NObjs),
  assign_func_value(continua(NObjs)).

% ultimo producto de lista
ultima_carta([H|T]):-
print(H),
assign_func_value(test(H)).
